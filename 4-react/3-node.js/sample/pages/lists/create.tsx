...
import { requestAddListPaging } from "../../middleware/modules/list";
...
const ListCreate = () => {
  ...
  const isAddCompleted = useSelector(
    (state: RootState) => state.list.isAddCompleted
  );
  ...
  useEffect(() => {
    ...
    // true이면 화면이동
    isAddCompleted && router.push("/lists");
  ...
  const handleAddClick = () => {
    ...
    if (fileInput.current?.files?.length) {
      ...
      reader.onload = () => {
        ...
        // state에 데이터 추가
        // 1. addlist함수에서 Action 객체를 생성함
        //    -> {type:"list/addlist", payload:item}
        ...
        /* ----- saga action으로 대체 ----- */
        // dispatch(requestAddlist(item)); // 전체조회
        dispatch(requestAddListPaging(item)); // 넘버 페이징
        // dispatch(requestAddListNext(item)); // 더보기 페이징
  ...
  return (
    <Layout>
      <section style={{ width: "40vw" }} className="mx-auto">
        <Progress />
        <AlertStack />
        <h2 className="text-center">List Create</h2>
        <form>
