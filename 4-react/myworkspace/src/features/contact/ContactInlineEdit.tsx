...
const Contact = () => {
  ...
  const add = async (e: React.KeyboardEvent<HTMLInputElement> | null) => {
    ...
    try {
      ...
      // const result = await api.add({ name: "" }); // 강제로 오류 발생 시켜봄
      console.log(result);
      ...
      setContactList(
        ...
        produce((state) => {
          // draft state 배열에 추가
          // draft state의 타입은 ContactItemState[]
  ...
  const save = async (id: number, index: number) => {
    ...
    // -------------- 백엔드 연동 부분 ----------------------
    if (!input) return;
    const result = await api.modify(id, { name: input.value });
      ...
      <ul id="ul-list" className="list-group list-group-flush mt-3" ref={ulRef}>
        ...
        {contactList.map((item, index) => (
          <li className="list-group-item d-flex" key={item.id}>
            <div className="w-100">
              {/* 보기모드일 때 보이는 내용 */}
              {!item.isEdit && <span className="me-1">{item.name}</span>}
              {!item.isEdit && (
