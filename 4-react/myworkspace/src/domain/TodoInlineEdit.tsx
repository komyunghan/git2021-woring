...
const Todo = () => {
  ...
  // [] 의존변수목록이 빈 배열 -> 컴포넌트가 렌더링되고 마운팅 된 후에 시점에 처리가 됨
  useEffect(() => {
    ...
    fetch("http://localhost:8080/todos")
      ...
      .then(res => res.json())
      // 응답데이터를 js object(array)로 변환이 완료되면 다음 then에 있는 함수(callback)를 실행함
      // then에 있는 callback 함수의 매개변수로 변환된 결과를 넘겨줌
      // **서버에서 데이터를 받아오는 구조에 대한 interface를 변도로 정의하는 것이 좋음
      .then((data: TodoItemResponse[]) => {
        // const todos = data as TodoState[]; // 타입 단언(Type AsserTions)
